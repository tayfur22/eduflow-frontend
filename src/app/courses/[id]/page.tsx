...
// ✅ FILE → QUIZ olaraq backend ilə uyğunlaşdırıldı
...
export default function TeacherCourseManagePage() {
  ...
  // ✅ videoUrl → contentUrl, description → textContent (backend field adları)
  ...
  const openAddLesson = (sectionId: number) => {
    ...
    // ✅ contentUrl, textContent — backend field adları
    setLessonForm({
      title: "",
      textContent: "",
      contentUrl: "",
  ...
  const addLesson = async () => {
    ...
    try {
      // ✅ contentUrl — backend CreateLessonRequest field adı
      const payload = {
  ...
  const lessonTypeIcon = (type: string) => {
    ...
    if (type === "QUIZ") return <ClipboardList size={13} color="#059669" />;
          ...
          <div className="card animate-fade-up" style={{ width: "100%", maxWidth: 580, padding: 32, margin: "auto" }}>
            ...
            <div style={{ display: "flex", flexDirection: "column", gap: 18 }}>
              ...
              <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
                ...
                />
                <label htmlFor="freePreview" style={{ fontSize: 13, color: "var(--text-secondary)", cursor: "pointer" }}>
                  Ön baxış — qeydiyyatsız görünür
                </label>
              </div>

              {/* Video/PDF - URL ya da Upload */}
              {(lessonForm.lessonType === "VIDEO" || lessonForm.lessonType === "PDF") && (
                <div>
                  <label style={{ fontSize: 13, fontWeight: 600, color: "var(--text-secondary)", display: "block", marginBottom: 10 }}>
                    {lessonForm.lessonType === "VIDEO" ? "Video mənbəyi" : "PDF mənbəyi"}
                  </label>

                  {/* Toggle: URL vs Upload — yalnız VIDEO üçün */}
                  ...
                  {/* File Upload (VIDEO fayl ya da PDF) */}
                  {(lessonForm.lessonType === "PDF" || (lessonForm.lessonType === "VIDEO" && uploadMode === "file")) && (
                    <div>
                      <input
                        ref={fileInputRef}
                        type="file"
                      ...
                      {!uploadFile ? (
                        ...
                        >
                          ...
                          <p style={{ fontSize: 14, fontWeight: 600, marginBottom: 4 }}>
                            {lessonForm.lessonType === "VIDEO" ? "Video fayl seçin" : "PDF fayl seçin"}
                          </p>
                          <p style={{ fontSize: 12 }}>
                            {lessonForm.lessonType === "VIDEO" ? "MP4, WebM, MOV" : "PDF"}
              ...
              {lessonForm.lessonType === "TEXT" && (
                <div>
                  ...
                  <textarea className="input"
                    placeholder="Dərs məzmununu buraya yazın — izah, kod nümunəsi, qeydlər..."
                    rows={8}
                    // ✅ textContent — backend field adı
